...
import { fetchUsuarioConRolPorDni, fetchUsuarioPorDni } from "@/lib/data";
...
export default async function UserModificationDashboard({ params }: { params: { 'id-usuario': string } }){
    ...
    const usuarioConRol: Usuario = await fetchUsuarioConRolPorDni(dni);

    return(
        <>
            <div>Modificar Usuario</div>
            <ModificationForm usuario={usuarioConRol} />
        </>
    );
}